...
// throw exception unless n >= m >= 0
	 ...
	 * next permutation to return. For example, an index on 
	 * a permutation of 3 things might contain {1 2 0}. This
	 * index will be followed by {2 0 1} and {2 1 0}.
	...
	/**
	 * The elements from m to n are always kept ascending 
	 * right to left. This keeps the 'dip' in the 
	 * interesting region.	 
	 */	
	reverseAfter(m - 1);
}
...
protected void moveIndex() {
	...
	for (int j = i + 2; j < n; j++) {
		if (index[j] < index[leastToRightIndex] && 
			 index[j] > index[i]) {
			leastToRightIndex = j;
		}	
	}
...
/**
 * @return Object, the next permutation of the original
 * Object array. 
 * <p>
